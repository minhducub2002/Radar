...
void setup() {
    ...
    myPort.clear();
    myPort.bufferUntil('\n');
    orcFont = loadFont("AgencyFB-Bold-30.vlw");
}
void draw() 
{
    fill(98, 245, 31);
...
{
    arc(0, 0, coefficient * width, coefficient * width, PI, TWO_PI); //vẽ nửa đường tròn
}
void DrawRadarAngledLine(final int angle){
    line(0, 0, (-width/2) * cos(radians(angle)), (-width/2) * sin(radians(angle))); //vẽ các đường thẳng
}
void DrawObject() 
{
    pushMatrix();
